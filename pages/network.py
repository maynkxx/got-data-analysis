...
# ── Keep only top 120 characters by screen time ────────────
top_n = 120
top_chars_list = (
...
# ── Build co-occurrence edges (top chars only) ─────────────
...
# ── Build lookup for character info ───────────────────────
chars_lookup = {}
...
# Determine which chars are actually in edges
chars_in_graph = set()
...
for char in chars_in_graph:
    ...
    # Node size: scaled between 12 and 55
    ...
    # Dead characters get faded rgba color
    color = base_color if is_alive == 1 else hex_to_rgba(base_color, 0.35)

    # Only show label for characters with >8000s screen time
